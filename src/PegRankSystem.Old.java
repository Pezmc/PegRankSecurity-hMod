...
public class PegRankSystem extends Plugin
{
	private static String name = "PegRankSystem";
	...
	public void enable() {		
		PropertiesFile properties	= new	PropertiesFile("PegRankSystem.properties");
		try {
			...
			dbUsername = properties.getString("username", dbUsername);
			dbPassword = properties.getString("password", dbPassword);
			delay = properties.getInt("delay", 10);
		} catch (Exception ex) {
			log.log(Level.SEVERE, name + ": exception while reading from PegRankSystem.properties",	ex);
			return;
		}		
		try {
			if (!usehModDb)
		...
		new Thread(consumer).start();
	      etc.getInstance().addCommand("/lb", " - PegRankSystem display command.");
	      etc.getInstance().addCommand("/rollback", " - PegRankSystem Rollback command.");
	   ...
	   public boolean onCommand(Player player, String[] split)
	   {
		...
		public boolean onBlockBreak(Player player, Block b)
        {
            ...
            try 
            {
                ...
                if (groupuser != null) {
                    if (groupuser.equals("admin"))
                        leveluser = 4;
                    else if (groupuser.equals("mod"))
                        leveluser = 3;
                    else if (groupuser.equals("builder"))
                        leveluser = 2;
                    else if (groupuser.equals("default"))
                        leveluser = 1;
                }
                ...
                if (groupowner != null) {
                    if (groupowner.equals("admin"))
                        levelowner = 4;
                    else if (groupowner.equals("mod"))
                        levelowner = 3;
                    else if (groupowner.equals("builder"))
                        levelowner = 2;
                    else if (groupowner.equals("default"))
                        levelowner = 1;
                }
                ...
                {
                    player.sendMessage("Your group is too low to remove that block!");
                    return true;
                }
		...
		{
			//Only water and lava are of interest
			if (item.getItemId() != 326 && item.getItemId() != 327) return false;
